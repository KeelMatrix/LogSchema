...
The installed-tool smoke is a separate gate. It creates a local tool manifest, installs the newly packed `KeelMatrix.LogSchema` package from an isolated feed and cache, copies only the manifest into a fresh checkout directory, runs `dotnet tool restore`, and invokes every command through `dotnet tool run logschema`. Its fail-closed matrix tampers each redundant event-identity dimension independently and requires installed `check` and `diff` to return the JSON analysis-error envelope and exit code 3 without an absolute path or stack trace.
