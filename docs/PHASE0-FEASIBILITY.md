...
The installed-tool smoke is a separate gate. It creates a local tool manifest, installs the newly packed `KeelMatrix.LogSchema` package from an isolated feed and cache, copies only the manifest into a fresh checkout directory, runs `dotnet tool restore`, and invokes every command through `dotnet tool run logschema`. Its fail-closed matrix tampers each redundant event-identity dimension independently, including additive values and positional parameter forms. It also runs symmetric forged-vs-forged `diff` cases for ordinary-string-as-exception and derived-exception-as-ordinary contradictions, plus comparison-originated form-error `check` and `diff` cases. Every case must return the JSON analysis-error envelope, `coverageComplete: false`, and exit code 3 without an absolute path or stack trace. Exact `System.Exception`, a non-suffix derived exception, and an ordinary custom type are captured with their distinct canonical semantic forms and round-trip cleanly.
