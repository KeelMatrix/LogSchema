...
Structured identity fields are compared with exact ordinal semantics. The reader recomputes every parameter form from its canonical declared type: `ILogger` for logger interfaces, `LogLevel` for exact `Microsoft.Extensions.Logging.LogLevel`, `Exception` for exact `System.Exception`, and `None` for every other type, including derived exceptions. Both the embedded form and redundant `parameterForms` position must equal that result, so coordinated, one-sided, and additive form forgeries fail as analysis errors. Derived exception events remain supported; capture records their declared type with form `None` and uses semantic inheritance only to exclude exception parameters from message placeholders. Case-only changes to placeholder names are structured renames (`KMLOG102`), while `KMLOG301` is reserved for prose changes whose structured shape is unchanged under ordinal comparison.
