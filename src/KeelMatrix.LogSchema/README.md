...
Each canonical parameter identity embeds a semantic form (`None`, `Exception`, `ILogger`, or `LogLevel`) beside its ref kind and canonical type. The redundant positional `parameterForms` vector must match that parsed evidence exactly, so identical forged manifests cannot hide an ordinary type labeled as an exception or a captured derived exception labeled as ordinary. Malformed or contradictory identity tuples, unknown parameter vocabularies, and comparison-time semantic-form disagreements are analysis errors: text and JSON return exit code 3 with no findings and `coverageComplete: false`.
