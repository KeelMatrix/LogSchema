...
Manifest identities must agree exactly with their redundant containing type, method, generic arity, parameter count, ref kinds, and positional parameter forms; additive values are rejected too. Malformed or contradictory identity tuples and unknown parameter vocabularies are analysis errors: text and JSON use their documented analysis-error envelopes and return exit code 3 before comparison.
